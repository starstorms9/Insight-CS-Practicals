...
class Sweeper() :
    def __init__(self, rows=5, cols=5, numbombs=5) :
        self.rows = rows
        self.cols = min(26, cols)
        self.board = self.Board(rows,cols, numbombs)
    ...
    def resetGame(self) :
        self.board = self.Board(self.rows,self.cols, self.num_bombs)
